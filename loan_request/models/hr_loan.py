...
class HrLoan(models.Model):
    ...
    job_position = fields.Many2one('hr.job', related="employee_id.job_id", readonly=True, string="Job Position",
                                   help="Job position")
    loan_amount = fields.Float(string="Loan Amount", required=False, help="Loan amount")
    ...
    loan_amount_journal_id = fields.Many2one(comodel_name="account.journal", string="", )
    loan_amount_debit_account_id = fields.Many2one(comodel_name="account.account", string="", )
    loan_amount_credit_account_id = fields.Many2one(comodel_name="account.account", string="", )
    loan_amount_account_move_id = fields.Many2one(comodel_name="account.move", string="", readonly=True)
    contract_id = fields.Many2one('hr.contract', related='employee_id.contract_id')
    contract_date_end = fields.Date(string="Contract End Date", required=False, related='contract_id.date_end')
    ...
    def create_account_move(self):
        move_line_1 = {
            'name': self.name or '',
            'account_id': self.loan_amount_debit_account_id.id or False,
        ...
        move_line_2 = {
            'name': self.name or '',
            'account_id': self.loan_amount_credit_account_id.id or False,
            'debit': 0.0,
            'analytic_account_id': self.employee_id.contract_id.analytic_account_id.id or False,
        ...
        move_vals = {
            'name': self.name or '',
            'date': self.payment_date or False,
            'state': 'draft',
            'journal_id': self.loan_amount_journal_id.id or False,
            'loan_id': self.id,
            'line_ids': [(0, 0, move_line_1), (0, 0, move_line_2)],
        }
        account_move = self.env['account.move'].create(move_vals)
        self.loan_amount_account_move_id = account_move.id
    ...
    @api.constrains('_check_payment_date')
    def _check_payment_date(self):
        for record in self:
            if record._check_payment_date:
                if record.payment_date > record.contract_date_end:
                    raise ValidationError(_('Your Payment Date Cant be more than Contract End Date.'))

    @api.constrains('_check_loand_ids')
    def _check_loand_ids(self):
        for record in self:
            if record.loan_lines:
                last_loan_date = record.loan_lines[-1].date
                if last_loan_date > record.contract_date_end:
                    raise ValidationError(_('Your Last Loan Date Cant be more than Contract End Date.'))
