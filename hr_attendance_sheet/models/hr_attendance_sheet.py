...
class AttendanceSheet(models.Model):
    ...
    no_overtime = fields.Integer(compute="calculate_att_data",
                                 string="No of overtimes", readonly=True,
                                 store=True)
    tot_overtime = fields.Float(compute="calculate_att_data",
                                string="Total Over Time", readonly=True,
                                store=True)
    tot_difftime = fields.Float(compute="calculate_att_data",
                                string="Total Diff time Hours", readonly=True,
                                store=True)
    no_difftime = fields.Integer(compute="calculate_att_data",
                                 string="No of Diff Times", readonly=True,
                                 store=True)
    tot_late = fields.Float(compute="calculate_att_data",
                            string="Total Late In", readonly=True, store=True)
    no_late = fields.Integer(compute="calculate_att_data", string="No of Lates",
                             readonly=True, store=True)
    no_absence = fields.Integer(compute="calculate_att_data",
                                string="No of Absence Days", readonly=True,
                                store=True)
    tot_absence = fields.Float(compute="calculate_att_data",
    ...
    bonus_request = fields.Float(string='Total Bonus')
    ...
    loan_deductions = fields.Float(string="Loans")
    ...
    def update_current_total_bonus_request_allowance(self):
        ...
        total_amount = 0
        bonus_objects = self.env['bonus.request'].search([('employee_id', '=', employee.id), ('request_date', '>=', date_from), ('request_date', '<=', date_to), ('state', '=', 'confirmed')])
    ...
    def update_current_penalty_to_paid(self):
        ...
        date_to = self.date_to
        total_amount = 0
        domain = [('employee_id', '=', employee.id), ('state', '=', 'confirmed')]
    ...
    def update_all_allowance_and_deduction_fields(self):
        ...
        if contract_object:
            ...
            self.social_insurance_deductions = contract_object[0].social_insurance_deductions
            self.medical_insurance_deductions = contract_object[0].medical_insurance_deductions
            self.fingerprint_deductions = contract_object[0].fingerprint_deductions
            self.administrative_deductions = contract_object[0].administrative_deductions
            self.absence_without_permission_deductions = contract_object[0].absence_without_permission_deductions
            self.absence_discount_without_credit_deductions = contract_object[0].absence_discount_without_credit_deductions
            ...
            self.motivation = contract_object[0].motivation
        else:
            self.general_deductions = 0.0
            self.social_insurance_deductions = 0.0
            self.medical_insurance_deductions = 0.0
            self.fingerprint_deductions = 0.0
            self.administrative_deductions = 0.0
            self.absence_without_permission_deductions = 0.0
            self.absence_discount_without_credit_deductions = 0.0
    ...
    def update_current_loan_to_paid(self):
        ...
        print("===================================")
        self.loan_deductions = total_amount
    ...
    def unlink(self):
        if any(self.filtered(
            ...
            raise UserError(_(
                'You cannot delete an attendance sheet which is not draft or confirmed!'))
        ...
        for att_sheet in self:
            ...
            if not tz:
                raise exceptions.Warning(
                    "Please add time zone for the current user %s" % self.env.user.name)
            ...
            for day in all_dates:
                ...
                tz_info = fields.Datetime.context_timestamp(self,
                        ...
                        for i, work_interval in enumerate(work_intervals):
                            ...
                            if att_work_intervals:
                                if len(att_work_intervals) > 1:
                                    ...
                                    late_in_interval = (
                                    work_interval[0], att_work_intervals[0][0])
                                    overtime_interval = (
                                    work_interval[1], att_work_intervals[-1][1])
                                    if overtime_interval[1] < overtime_interval[
                                        ...
                                        overtime = overtime_interval[1] - \
                                                   overtime_interval[0]
                                    remain_interval = (
                                    att_work_intervals[0][1], work_interval[1])
                            ...
                            if float_overtime <= overtime_policy['wd_after']:
                                ...
                                act_float_overtime = (
                                            float_overtime - overtime_policy[
                                        'wd_after'])
                                float_overtime = float_overtime * \
                                                 overtime_policy[
                                                     'wd_rate']
    ...
    def create_payslip(self):
        ...
        for att_sheet in self:
            ...
            absence_without_permission_deductions = [{
                'name': "absencewithoutpermissiondeductions",
                'code': 'ABSWIPERMDED',
                'contract_id': contract_id,
                'sequence': 85,
            ...
            absence_discount_without_credit_deductions = [{
                'name': "absencediscountwithoutcreditdeductions",
                'code': 'ABSDISCDED',
                'contract_id': contract_id,
                'sequence': 55,
                'number_of_days': att_sheet.absence_discount_without_credit_deductions,
                'number_of_hours': att_sheet.absence_discount_without_credit_deductions,
            }]

            loan_deductions = [{
                'name': "loanrequestdeduction",
                'code': 'LOANS',
                'contract_id': contract_id,
                'sequence': 60,
                'number_of_days': att_sheet.loan_deductions,
                'number_of_hours': att_sheet.loan_deductions,
            }]
            ...
            motivation = [{
                'name': "motivationallowance",
                'code': 'MOTIVATIONALLOW',
                'contract_id': contract_id,
                'sequence': 95,
            ...
            worked_days_line_ids += overtime + late + absence + difftime + bonus + penalty + general_deductions + social_insurance_deductions + medical_insurance_deductions + fingerprint_deductions + administrative_deductions + absence_without_permission_deductions + absence_discount_without_credit_deductions + loan_deductions + house_allowances + overtime_allowance + treatment_allowance + transport_allowances + living_allowances + telephone_allowance + motivation
