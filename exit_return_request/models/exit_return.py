...
class ExitReturn(models.Model):
    _name = 'exit.return'
    _inherit = ['mail.thread', 'mail.activity.mixin']

    name = fields.Char(string="Name", default="New", readonly=True, copy=False)

    state = fields.Selection(string="", selection=[('draft', 'Draft'), ('d_manager', 'Direct Manager'), ('hr_manager', 'HR Manager'), ('confirm', 'Confirmed'), ('rejected', 'Rejected')], required=False, default='draft')
    ...
    type = fields.Selection(string="Type", selection=[('multi', 'Multi'), ('single', 'Single'), ], required=True, )
    ...
    country_id = fields.Many2one(comodel_name="res.country.state", string="Country Name", required=True, )
    ...
    def create(self, vals):
        vals['name'] = (self.env['ir.sequence'].next_by_code('exit.return')) or 'New'
        return super(ExitReturn, self).create(vals)

class AccountMoveInherit(models.Model):
    _inherit = 'account.move'

    exit_return_id = fields.Many2one(comodel_name="exit.return", string="", required=False, )